        ...
        try {
            for (int i = 0; i < VALID_BEAN_PROPERTIES.length; i++) {
                ...
                if (BeanUtils.getProperty(this.priestBean, VALID_BEAN_PROPERTIES[i]) != null) {
                    assignedValue = BeanUtils.getProperty(this.priestBean, VALID_BEAN_PROPERTIES[i]).toString();
